...
""")
def copycallback(textobj):
    pclip.copy(textobj.get("sel.first", "sel.last"))
def pastecallback(textobj):
    textobj.insert("insert", pclip.paste())
...
editmenu.add_command(label="Copy", command=(lambda: copycallback(buffer)))
editmenu.add_command(label="Paste", command=(lambda: pastecallback(buffer)))
